...
public class SaveWorld : MonoBehaviour
{
    ...
    private IEnumerator SaveWorldRoutine()
    {
        ...
        {
            Debug.LogWarning("[SaveWorld] Skipping save - no worldId.");
            yield break;
        }
        ...
        {
            Debug.LogWarning("[SaveWorld] Skipping save - no JWT token.");
            yield break;
        }
        ...
        {
            if (ok) Debug.Log("[SaveWorld] Auto-save successful.");
            else    Debug.LogWarning($"[SaveWorld] Auto-save failed: {json}");
        }));
    }
}
